...
public class HttpRequestLoggerInterceptor implements ClientHttpRequestInterceptor {
    ...
    private void doInterceptAfter(HttpRequest request, byte[] requestBody, ClientHttpResponse
        ...
        if (log.isInfoEnabled())
            log.info("拦截http请求，请求 URI ：{}，请求方法：{}，请求实体长度：{}，请求实体：{}，响应状态：{}",
                    requestURI, requestHttpMethod.name(), bodyContentToStringLength(requestBody),
                    bodyContentToString(requestBody, BODY_CONTENT_TO_STRING_MAX_LENGTH), new
                            StringBuilder().append(responseHttpStatus.name()).append("/").append
                            (responseHttpStatus.value()).toString());
    }
    ...
    private String bodyContentToString(byte[] body, int maxLength) {
        ...
        if (maxLength > 0 && (!StringUtils.isEmpty(tempBodyContentToString))) {
            bodyContentToString = tempBodyContentToString.substring(0, Math.min(maxLength,
                    tempBodyContentToString.length()));
        } else {
